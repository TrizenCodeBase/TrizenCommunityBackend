const mongoose = require('mongoose');
require('dotenv').config();

const Event = require('../models/Event');

async function testSpeakerSaveDirect() {
    try {
        await mongoose.connect(process.env.MONGODB_URI);
        console.log('✅ Connected to MongoDB');

        // Create a test event with speaker data exactly as it would come from frontend
        const testEventData = {
            title: 'Test Event with Speaker',
            description: 'Test description for speaker image test',
            category: 'Workshop',
            type: 'Online',
            startDate: new Date(),
            endDate: new Date(Date.now() + 24 * 60 * 60 * 1000),
            maxAttendees: 100,
            price: 0,
            organizer: new mongoose.Types.ObjectId(),
            speakers: [{
                name: 'Test Speaker',
                title: 'Test Title',
                company: 'Test Company',
                bio: 'Test bio',
                image: 'data:image/jpeg;base64,',
                socialLinks: {
                    linkedin: '',
                    twitter: '',
                    website: ''
                }
            }]
        };

        console.log('🔍 Test event data:', testEventData);
        console.log('🔍 Speakers data:', testEventData.speakers);
        console.log('🔍 First speaker image length:', testEventData.speakers[0].image.length);

        const event = new Event(testEventData);
        console.log('🔍 Event instance created');
        console.log('🔍 Event speakers:', event.speakers);
        console.log('🔍 First speaker in event:', event.speakers[0]);
        console.log('🔍 First speaker image in event:', event.speakers[0].image);

        await event.save();
        console.log('💾 Event saved successfully');

        // Reload from database
        const savedEvent = await Event.findById(event._id);
        console.log('💾 Event reloaded from database');
        console.log('💾 Saved speakers:', savedEvent.speakers);
        console.log('💾 First saved speaker:', savedEvent.speakers[0]);
        console.log('💾 First saved speaker image:', savedEvent.speakers[0].image);
        console.log('💾 First saved speaker image length:', savedEvent.speakers[0].image?.length);

        // Clean up
        await Event.findByIdAndDelete(event._id);
        console.log('🧹 Test event deleted');

    } catch (error) {
        console.error('❌ Error:', error);
    } finally {
        await mongoose.disconnect();
    }
}

testSpeakerSaveDirect();


